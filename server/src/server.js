    ...
    if (fromUser === userId) {
      db.collection('feedItems').updateOne({ _id: feedItemId },
      {
        ...
        db.collection('feedItems').findOne({ _id: feedItemId }, function(err, feedItem) {
          ...
          resolveUserObjects([feedItem.comments[commentIdx].author].concat(feedItem.comments[commentIdx].likeCounter), function(err, userMap) {
            if (err) {
              return sendDatabaseError(res, err);
            }
            // Return a resolved version of the likeCounter
